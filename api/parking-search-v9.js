...
function searchTerms(q = '') {
  ...
  for (const [key, list] of Object.entries(CATEGORY_TERMS)) {
    if (x.includes(key) || key.includes(x)) list.slice(0, 4).forEach((term) => terms.add(term));
  }
...
async function jsonFetch(url, timeoutMs = 5200) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { Accept: 'application/json', 'User-Agent': 'ParkLink v9 search' } });
...
async function nominatimSearch(term, req, limit = 5) {
  ...
  return jsonFetch(`https://nominatim.openstreetmap.org/search?${p}`, 5000).catch(() => []);
}
...
async function placesMode(req, res) {
  ...
  const suggestions = rows
    ...
    .slice(0, 10);
  return res.status(200).json({ suggestions, place: suggestions[0] || null, mode: 'v9-fast' });
}
...
async function overpassParking(place, minutes) {
  ...
  const response = await fetch('https://overpass-api.de/api/interpreter', { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=UTF-8', 'User-Agent': 'ParkLink v9 parking' }, body: q });
  if (!response.ok) return { lots: [], streets: [], streetNote: 'Street parking data unavailable from map source.' };
  ...
  const seen = new Set();
  const lots = elements.filter((item) => item.tags?.amenity === 'parking').map((item) => {
    const lat = item.lat ?? item.center?.lat;
    const lng = item.lon ?? item.center?.lon;
  ...
  const streets = roads.map((road, index) => {
    ...
    const hasMappedParking = Boolean(
      road.tags['parking:lane:both'] || road.tags['parking:lane:left'] || road.tags['parking:lane:right'] ||
      road.tags['parking:both'] || road.tags['parking:left'] || road.tags['parking:right'] ||
      road.tags['parking:condition:both'] || road.tags['parking:condition:left'] || road.tags['parking:condition:right']
    );
    if (!hasMappedParking) return null;
    ...
    return {
      ...
      bestLot: `${road.name} • map data marks parking on this street; verify signs/meters`,
      distance,
      capacity: 0,
      price: 'Check posted rules',
      walk: `${Math.max(1, Math.round(distance * 18))} min walk`,
      reason: `Parking-related street tags exist on ${road.name}. Still verify posted signs, meters, red curbs, permits, and event restrictions.`,
      lat: road.lat,
      lng: road.lng,
      mapQuery: `${road.name} near ${basePlaceName(place.name)}`,
      kind: 'street',
      source: 'Mapped street-parking data',
      accessibility: 'Curb accessibility not confirmed',
      priority: 8
    };
  }).filter(Boolean).sort((a, b) => a.distance - b.distance).slice(0, 6);
  return { lots, streets, streetNote: streets.length ? '' : 'No verified mapped street-parking segments found nearby. This avoids guessing; check signs/meters locally.' };
}
...
async function parkingMode(req, res) {
  ...
  const [mapped, curated] = await Promise.all([
    overpassParking(place, minutes).catch(() => ({ lots: [], streets: [], streetNote: 'Street parking data unavailable from map source.' })),
  ...
  return res.status(200).json({
    ...
    streetNote: mapped.streetNote,
    info: 'Destination/customer lots are shown first, then real mapped lots, then only verified mapped street-parking segments. No street parking is guessed.'
  });
}
